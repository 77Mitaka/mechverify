...
\bigskip
\noindent\textbf{Abstract:} Planar mechanism design still relies heavily on engineering experience and lacks a textual, automatically verifiable and data-accumulating carrier. Inspired by the success of program code (massive paired corpora plus a cheap automatic verifier), we propose a verifiable description language (a mechanism intermediate representation, IR) and a layered automatic verifier for planar mechanisms, and build a generate--verify--persist data loop. The IR unifies topology, geometry, actuation and requirements; the verifier filters candidates layer by layer according to cost: Layer~0 structural checks (mobility, Grashof, closure feasibility), Layer~1 analytic/constraint-based kinematics (loop closure, assembly-branch selection, singularity detection), Layer~2 kinematic simulation in Modelica (self-contained and PlanarMechanics backends), and Layer~3 Monte-Carlo robustness under manufacturing tolerances. The verifier exposes a unified interface returning status, score, per-constraint checks and trajectories, and serves as the oracle of the data loop. On four-bar, slider-crank and Watt six-bar linkages, the PlanarMechanics simulation matches the analytic solution to the order of $10^{-11}$; the straight-line four-bar passes nominally but only $83\%$ of samples under $\pm0.2\%$ tolerance; one run of the data loop persists 300 (requirement, mechanism, score) samples. The framework reproduces the ``paired requirement/implementation, stamped by a verifier'' mechanism of the code domain and provides data and evaluation foundations for data-driven mechanism synthesis.

\noindent\textbf{Keywords:} planar mechanism; mechanism synthesis; Modelica; PlanarMechanics; automatic verification; data-driven design
...
机构学对机构的拓扑表示早有系统研究。Freudenstein 与 Maki 将运动链抽象为图论模型，以构件为节点、运动副为边，用邻接矩阵描述机构拓扑，并据此开展类型综合。KINSYN、LINCAGES 等早期计算机辅助机构综合程序已实现``输入尺寸参数—求解—绘制机构''的闭环，但受限于时代，其数据格式未能形成通用语言与生态。Erdman 与 Sandor 系统总结了平面机构的分析与综合方法，包括 Grashof 条件、传递角、死点判定等，为自动验证提供了经典理论基础。此外，基于数值优化的机构综合（如遗传算法\cite{cabrera2002}）以目标函数驱动尺寸寻优，但通常不提供统一的、可复用的验证判据。

\subsection{Modelica 与多体仿真}
Modelica 是面向对象且基于方程的跨领域物理建模语言，其``非因果''特性使编译器在全局完成因果分配与方程求解，天然支持闭环机构。PlanarMechanics（DLR）面向二维平面机构，提供 Body、FixedTranslation、Fixed、Revolute、Prismatic 等组件与 Frame 连接器，可拖拽建模并动画仿真；OpenModelica\cite{fritzson2020} 提供开源编译器，FMI\cite{fmi} 支持模型跨工具交换。多体动力学引擎（MuJoCo、ADAMS、Simscape Multibody）为机构运动学与动力学提供成熟数值手段。然而，这些工具本质上是``仿真模型语言/环境''，其抽象层级低于工程师思考的``四杆机构''层次，且不回答``该机构是否满足设计需求''。
...
\subsection{现状与空白}
机构学的拓扑表示、Modelica 的方程求解、多体仿真的验证能力、代码领域的数据范式、图形化建模的交互方式均已各自成熟，但尚缺一个将它们统一起来的核心——面向平面机构原理图的``可验证描述语言 $+$ 自动验证器''。本文以此为切入点，其与已有工具/方法的定位差异见表~\ref{tab:diff}。

\begin{table}[htbp]
\centering
\caption{与已有工具/方法的定位差异}
\label{tab:diff}
\small
\begin{tabular}{p{2.4cm}p{3.2cm}p{3.2cm}p{4.4cm}}
\toprule
能力 & 通用仿真工具 & 机构综合程序 & 本文框架 \\
\midrule
描述载体 & 仿真模型(低层) & 尺寸参数/图形 & 机构 IR(拓扑+几何+驱动+需求) \\
自动验证 & 需人工配参数 & 无统一判据 & 四层、统一接口 \\
需求判据 & 无 & 单一目标函数 & 谓词表 + 分级评分 \\
批量/沉淀 & 无 & 无 & SQLite 语料库 \\
速度 & 秒级 & 迭代较慢 & 解析层 $0.2$\,ms \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{需求的形式化}
需求被形式化为对可观测量的谓词表，每项包含：观测量（如连杆点轨迹、传递角、奇异裕度）、运算符、目标值或目标路径、容差与权重。硬约束（如 Grashof 成立、传递角大于阈值）决定可行性，为布尔判定；软目标（如轨迹精度）以加权和聚合为评分。评分函数同时作为数据回路中生成模型的稠密奖励。当多个软目标相互冲突时，可采用加权和或 Pareto 前沿处理；本文实现加权和，Pareto 与约束多目标留作后续工作。

\section{分层自动验证器}
验证器以``逐级过滤''为设计原则：由零成本的结构检查到高成本的鲁棒性分析，逐层淘汰不合格方案，使绝大多数需求在廉价的解析层即可判定，仅在必要时调用昂贵的仿真。其主体并非 Modelica，而是一个自写的平面运动学求解器；Modelica 作为需要独立参照时的第二套运动学后端。总体流程如下。

\begin{center}
\fbox{\begin{minipage}{0.92\textwidth}
\textbf{算法 1} \texttt{verify(IR, req)}\\
\textbf{输入：}机构 IR、需求 \texttt{req}；\quad\textbf{输出：}状态、评分、逐项判据、轨迹。
\begin{enumerate}[leftmargin=1.8em,itemsep=1pt,topsep=2pt]
\item 第 0 层：结构检查；若失败，返回 \texttt{fail}。
\item 若为四杆：解析扫描；否则：约束多体求解（延续法选支、掉支检测、奇异裕度）。
\item 计算可观测量（轨迹、传递角等），按 \texttt{req} 谓词表评估：硬约束判可行性，软目标聚合成评分。
\item 若需求涉及精算：由 IR 生成 Modelica，调用 \texttt{omc} 仿真（第 2 层）。
\item 若需求涉及鲁棒性：蒙特卡洛（第 3 层）。
\item 返回 \texttt{\{status, score, checks, diagnostics, trace\}}。
\end{enumerate}
\end{minipage}}
\end{center}

\textbf{复杂度}：第 0 层 $O(n)$；第 1 层四杆为每步 $O(1)$，通用多体为 $O(N\cdot m^3)$（$N$ 为扫描步数，$m=3(n-1)$ 为广义坐标数，$m^3$ 来自牛顿法解线性系统）；第 2 层为编译器/求解器代价（秒级）；第 3 层为第 1 层的 $O(K)$ 倍（$K$ 为样本数）。
...
\subsection{第 2 层：运动学精算（Modelica）}
由 IR 生成 Modelica 模型，并以第 1 层构型构造一致初值，调用 OpenModelica 编译仿真，提取可观测量。需要说明的是，本层当前以\textbf{运动学}为主：输入以 \texttt{Position} 源精确驱动并关闭重力，尚未引入载荷/力矩等动力学量，因此它提供的是独立于自写求解器的第二套运动学参照，而非完整的动力学仿真。提供两种后端（见第 5 节）。仿真失败按 fail 处理并给出诊断，不中断整体流程。

\subsection{第 3 层：鲁棒性分析}
对制造误差（杆长偏差、铰点位置偏差）做蒙特卡洛扰动，统计需求通过率与各行为指标的分位数，检验需求在误差下是否仍成立（鲁棒设计的通用思想见文献\cite{beyer2007}）。
...
\subsection{性能与确定性}
表~\ref{tab:perf} 给出各层实测耗时。四杆解析扫描为 $0.2$\,ms/次，支撑大规模候选筛选；通用多体求解因采用数值雅可比与逐步牛顿迭代，约 $1.0$\,s/次，是后续优化重点；鲁棒性（100 样本）为 $0.035$\,s/次；Modelica 编译+仿真首次约 $9.7$\,s，仅用于最终候选。关于``不 flaky''：四杆验证重复运行 50 次得到唯一评分，结果确定可复现。测试套件含 40 个单元/集成测试。表~\ref{tab:conv} 给出直线四杆直线度随扫描步数的离散化收敛，最小传递角在所有步数下已收敛到 $58.42^\circ$。

\begin{table}[htbp]
\centering
\caption{各层实测耗时（Python 3.11，单线程）}
\label{tab:perf}
\begin{tabular}{lcc}
\toprule
阶段 & 方法 & 耗时 \\
\midrule
第 0+1 层 & 四杆解析扫描 & $0.2$\,ms/次 \\
第 1 层 & 通用多体（数值雅可比） & $1.0$\,s/次 \\
第 3 层 & 鲁棒性（100 样本，四杆） & $0.035$\,s/次 \\
第 2 层 & Modelica 编译+仿真（首次） & $9.7$\,s/次 \\
确定性 & 四杆验证重复 50 次 & 唯一评分 \\
\bottomrule
\end{tabular}
\end{table}
...
\section{讨论与局限}
本文框架已在低副平面机构上打通端到端闭环，但仍存在局限：(1) 尚未支持高副（齿轮、凸轮）；(2) 未做真机或高保真标定以量化 sim-to-real 误差；(3) 大规模验证的并行吞吐与离散拓扑搜索尚属长期方向；(4) 可视化前端采用复用现成工具（PlanarMechanics/OMEdit）而非自研。上述与代码领域``数据飞轮规模化''的差距，属于长期开放问题。此外需要澄清，本文定位为面向数据驱动机构综合的\textbf{基础设施}——提供可验证的数据与判据，但尚未训练学习模型，``自然语言 $\rightarrow$ 机构''的实际学习实验留作后续。
...
\section*{代码可用性}
本文的实现代码、示例机构、测试与一键复现脚本见：
\begin{center}
\url{https://github.com/77Mitaka/mechverify}
\end{center}
...
\bibitem{cabrera2002} Cabrera J A, Simon A, Prado M. Optimal synthesis of mechanisms with genetic algorithms. Mechanism and Machine Theory, 37(10):1165--1177, 2002.
\bibitem{beyer2007} Beyer H G, Sendhoff B. Robust optimization --- A comprehensive survey. Computer Methods in Applied Mechanics and Engineering, 196(33--34):3190--3218, 2007.
\bibitem{fritzson2020} Fritzson P, et al. The OpenModelica Integrated Environment for Modeling, Simulation, and Model-Based Development. Modeling, Identification and Control, 41(4):241--295, 2020.
\bibitem{fmi} Modelica Association. Functional Mock-up Interface (FMI) Standard, Version 2.0/3.0.
\end{thebibliography}
